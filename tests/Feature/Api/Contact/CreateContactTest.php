...
it('should return the email has already been taken', function () {
    ...
    $payload = Contact::factory()->make(['user_id' => $user->id, 'email' => $contact->email])->toArray();
...
it('should return the phone number has already been taken', function () {
    ...
    $payload = Contact::factory()->make(['user_id' => $user->id, 'phone_number' => $contact->phone_number])->toArray();
...
test('should return the cellphone number has already been taken', function () {
    $model = new Contact;
    $user = User::factory()->create();
    $contact = Contact::factory()->create(['user_id' => $user->id]);

    $payload = Contact::factory()->make(['user_id' => $user->id, 'cellphone_number' => $contact->cellphone_number])->toArray();
